 ...
 * A Token
 *
 * author: Solulab Inc. - Ujjwal Bajaj
 * 
...
abstract contract Tokenomics {
    ...
    string internal constant NAME = "A Token";
    ...
    address internal marketingAndOperatingWallet = 0x74A892AA1fc6c8C44018cDd16a597fb7151195d8;
    address internal supplierWallet = 0x841eE81FF407Ba5504e103D15D8028116391810d;

    enum FeeType { Rfi, Burn, MarketingAndOperating, Supplier }
...
abstract contract ReflectionToken is IBEP20, Ownable, Presaleable, Tokenomics {
    ...
    function _transferTokens(address sender, address recipient, uint256 amount, bool takeFee, uint256 sumOfFees) private {
        ...
        (uint256 rAmount, , uint256 tAmount, , uint256 currentRate ) = _getValues(amount, sumOfFees);
        ...
        // Deduct Seller's tax
        uint256 finalTransferAmount = _takeSellersTax( recipient, amount, currentRate, sumOfFees );           
    ...
    function _takeBuyersTax(uint256 amount, uint256 currentRate, uint256 sumOfFees ) private {
     ...
     */
    function _takeSellersTax(address seller, uint256 amount, uint256 currentRate, uint256 sumOfFees) internal virtual returns (uint256);

}

contract AToken is ReflectionToken {
    ...
    function _takeSellersTax(address seller, uint256 amount, uint256 currentRate, uint256 sumOfFees) internal override returns (uint256) {
        ...
        if (sumOfFees > 0 && !isInPresale) {
            ...
            if (sellerBalance < 8000 * ZEROES) {
                sellerTaxRate = 200;   
            } else if (sellerBalance >= 8000 * ZEROES && sellerBalance < 15000 * ZEROES) {
                sellerTaxRate = 150;
            } else if (sellerBalance >= 15000 * ZEROES && sellerBalance < 20000 * ZEROES) {
                sellerTaxRate = 20;
            } else {
                sellerTaxRate = 0;
            }
            ...
            if (_isExcludedFromRewards[seller] ) { _balances[seller] = _balances[seller].add(tAmount); }
            
            return tAmount;            
        } else {
            
            uint256 rTransferAmount = amount.mul(currentRate);
            _reflectedBalances[seller] = _reflectedBalances[seller].add(rTransferAmount);
            
            if (_isExcludedFromRewards[seller] ) { _balances[seller] = _balances[seller].add(amount); }
